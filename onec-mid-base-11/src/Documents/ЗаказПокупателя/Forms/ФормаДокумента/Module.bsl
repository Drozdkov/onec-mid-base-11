...
Процедура Дор_ПересчитатьСуммуСтроки(Строка) 
	...
	Строка.Сумма = Строка.Цена * Строка.Количество * (100 - Объект.Дор_СогласованнаяСкидка)/100;
...
Процедура ДобавитьПолеИКомандуСкидка()
	...
	НовыйЭлемент.УстановитьДействие("ПриИзменении", "Дор_СогласованнаяСкидкаПриИзмененииВместо");
	
	Команда = Команды.Добавить("Дор_ПересчитатьСУчетомСкидки");
	...
	КнопкаФормы.ИмяКоманды = "Дор_ПересчитатьСУчетомСкидки";
